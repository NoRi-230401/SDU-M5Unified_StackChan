...
#define VOLUME_TXT "/volume.txt"

extern bool USE_SERVO_ST;
extern int SERVO_PIN_X;
extern int SERVO_PIN_Y;
extern uint8_t VOLUME_VAL;

void SDU_lobby();
bool SdBegin();
bool servoTxtSDRead();
bool volumeTxtSDRead();

const String PROG_NAME = "stackchan-whisper-mp3";
...
bool servoTxtSDRead()
{
  ...
  Serial.println("------------------------");
  Serial.println("SERVO ON/OFF = " + SV_ON_OFF);
...
bool volumeTxtSDRead()
{
  ...
  File fs = SD.open(VOLUME_TXT, FILE_READ);
  if (!fs)
  {
    SD.end();
    return false;
  ...
  String VOLUME_STR = String(buf);
  int vol = VOLUME_STR.toInt();
  if (vol > 255)
    vol = 255;

  if (vol <= 0)
    vol = 0;
  VOLUME_VAL = (uint8_t)vol;
